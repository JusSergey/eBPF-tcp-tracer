...
use tcpk_common::{TcpEvent, TCP_EVENT_SEND_TEMPLATE};
...
pub unsafe fn sys_sendto_entry(ctx: ProbeContext) -> Result<u32, u32> {
    ...
    match get_tmp_event(&ctx, bpf_get_current_pid_tgid()) {
        Ok(event) => {
            ...
            *event = TCP_EVENT_SEND_TEMPLATE;
            ...
            } = &mut *event
            {
                *connection = origin_connection;
                payload.size = len; // overrides at kretprobe
    ...
    match get_tmp_event(&ctx, tid) {
        Ok(event) => {
            ...
            } = &mut *event
            {
                ...
                //
                // let current_portion = core::cmp::min(left, size_of_buff);
                // info!(&ctx, "SENT {}", current_portion);
                // let a:[u8;6] = bpf_probe_read_kernel(payload.kernel_ptr as *const [u8; 6]).map_err(|_| 0u32)?;
                ...
                // info!(&ctx, "REALLYSENT {}", current_portion);
                // // for _ in 0..((64 * 1024) / size_of_buff) {
                // //     let left = ret as usize - processed;
                // //     // if left == 0 {
                // //     //     break;
                // //     // }
